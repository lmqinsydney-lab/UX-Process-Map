...
function ModuleCard({
  ...
  // 只展示本页面存在对应形态的模块状态（当前生效态 + 有页面状态标注的态）
  const reachableStates = mod.states.filter(
    (s) => s.id === modState || page.states.some((ps) => ps.moduleStates?.[inst.moduleId] === s.id),
  )
  ...
  return (
    ...
    >
      ...
      {(!inState || reachableStates.length > 1) && (
